...
impl Waveform {
    ...
    pub fn new(sample_rate_hz: f64,
               ...
               ctype: ComplexType,
               windowing: WindowingType,
               pct_unwindowed: f32) -> Waveform
        
    {
        let mut wf = Waveform{ sample_rate_hz: sample_rate_hz,
    ...
    fn update(&mut self) {
        ...
        let num_windowed_samples =
            num_samples * (1.0 - self.pct_unwindowed as f64) / 2.0;
...
impl fmt::Display for Waveform {
    fn fmt (&self, f: &mut fmt::Formatter) -> fmt::Result {
        try!(write!(f, "Waveform:\n"));
        try!(write!(f, "  Sample Rate Hz:  {}\n",
                    self.sample_rate_hz));
        try!(write!(f, "  Start Freq Hz:   {}\n",
                    self.start_freq_hz));
        try!(write!(f, "  Stop Freq Hz:    {}\n", self.stop_freq_hz));
        try!(write!(f, "  Length Seconds:  {}\n", self.length_sec));
        try!(write!(f, "  Total Samples:   {}\n", self.num_samples));
        try!(write!(f, "  Start Phase Deg: {}\n", self.start_phase_deg));
        try!(write!(f, "  Complex Type:    {:?}\n", self.ctype));
        try!(write!(f, "  Windowing:       {:?}\n", self.windowing));
        try!(write!(f, "  Windowed Samples:{}\n", self.num_windowed_samples));
